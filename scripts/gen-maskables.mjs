...
async function maskable(src, size, safeScale, out) {
  ...
  await sharp({ create: { width: size, height: size, channels: 4, background: "#020617" } })
    .composite([{ input: innerBuf, gravity: "center" }])
    .png()
    .toFile(out);
}

await maskable(base192, 192, 0.80, `${outDir}/doggerz-maskable-192.png`);
await maskable(base512, 512, 0.80, `${outDir}/doggerz-maskable-512.png`);
