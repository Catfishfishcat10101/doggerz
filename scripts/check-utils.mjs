...
(async function run() {
  ...
  try {
    const weather = await fetchWeatherByZip("65401").catch((e) => ({
      error: e.message,
    }));
  ...
  console.log(
    "getSpriteForStageAndTier (PUPPY):",
    getSpriteForStageAndTier("PUPPY"),
  );
  console.log(
    "getSpriteForStageAndTier (dog obj):",
    getSpriteForStageAndTier({ stage: "ADULT", cleanlinessTier: "FRESH" }),
  );
