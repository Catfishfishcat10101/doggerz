...
/**
 * Canonical map of all supported dog animation keys.
 * Use these keys for sprite selection, animation state, and alias resolution.
 */
...
/**
 * Default animation to fallback to when resolution fails.
 */
export const DEFAULT_DOG_ANIMATION = DOG_ANIMATIONS.idle;

// Maps alternate, legacy, or synonym animation keys to canonical DOG_ANIMATIONS values.
// This allows the engine and UI to resolve various user or system-provided animation names
// to a supported canonical animation for consistent rendering and behavior.
...
/**
 * Normalizes an animation key to lowercase and removes whitespace.
 */
...
export function resolveDogAnimation(value) {
  ...
  if (key in DOG_ANIMATIONS) return DOG_ANIMATIONS[key];
  if (key in DOG_ANIMATION_ALIASES) return DOG_ANIMATION_ALIASES[key];
...
export function isKnownDogAnimation(value) {
  const key = normalizeDogAnimationKey(value);
  return key in DOG_ANIMATIONS || key in DOG_ANIMATION_ALIASES;
}
