...
import { buildDoggerzProgressionPath } from "@/features/progression/doggerzProgressionPath.js";
...
export default function MainGame({ scene, dogInteractive = true }) {
  ...
  const handledCareResponseIdRef = useRef("");
  ...
  const trustScore = Math.round(
    Number(dog?.personalityProfile?.trust?.score || bondPct || 0)
  );
  const pottySequencePhase = String(
    dog?.potty?.sequence?.phase || ""
  ).toLowerCase();
  const pottySequenceLabel =
    pottySequencePhase === "cued"
      ? "Player cue active"
      : pottySequencePhase === "anxious_sniffing"
        ? "Anxious sniffing"
        : pottySequencePhase === "sniffing"
          ? "Sniffing"
          : pottySequencePhase === "rising"
            ? "Potty urge rising"
            : pottySequencePhase === "missed"
              ? "Missed cue"
              : "";
  const displayMoodHint = [moodPresentation?.hint || "", pottySequenceLabel]
    .filter(Boolean)
    .join(" ");
  ...
  const progressionPathModel = useMemo(
    () =>
      buildDoggerzProgressionPath({
        dog,
        progression,
        pottyTrack: pottyProgressionTrack,
        reliableCommandCount: progressionReliableCommandCount,
        masteredCommandCount: progressionMasteredCommandCount,
      }),
    [
      dog,
      pottyProgressionTrack,
      progression,
  ...
  useEffect(() => {
    const response = dog?.lastCareResponse;
    const responseId = String(response?.id || "").trim();
    if (!responseId || handledCareResponseIdRef.current === responseId) return;
    handledCareResponseIdRef.current = responseId;

    pushStageFeedback({
      key: response.key || "care-response",
      label: response.label || "Care",
      message: response.message,
      icon: response.icon || "*",
      tone: response.tone || "emerald",
    });
  }, [dog?.lastCareResponse, pushStageFeedback]);
  ...
  const actionOutcomeLabel = useMemo(() => {
    ...
    const labels = {
      ...
      potty_fakeout: "Potty outcome: false start. Circled, then settled.",
  ...
  const temperamentCardCopy = useMemo(() => {
    ...
    const archetypeCopy = {
      ...
      MISCHIEVOUS: {
        title: "72-Hour Archetype Read Ready",
        summary:
          "Your pup is reading as Mischievous: clever, distractible, and likely to test boundaries before settling.",
        detail:
          "Expect false starts, extra checking, and slower reliability until routines are consistent.",
      },
    };
    const byPrimary = {
      SWEET:
  ...
  const handleQuickFeed = useCallback(() => {
    ...
    try {
      if (!barkAudioRef.current) {
        barkAudioRef.current = new Audio(withBaseUrl("/audio/bark.m4a"));
      }
      const audio = barkAudioRef.current;
      audio.volume = clamp(masterVolume * sfxVolume, 0, 1);
      audio.currentTime = 0;
      audio.play().catch(() => {});
    } catch {
  ...
  const funnyShareMoment = useMemo(() => {
    ...
    const mapping = {
      ...
      potty_fakeout: "potty false start",
  ...
  const bottomMenuSections = useMemo(
    () => ({
      interact: {
        title: "Interactions",
        subtitle: "Daily care, comfort, and connection.",
        items: [
          {
            key: "feed",
            label: "Quick Feed",
            detail: "Meal when hunger builds",
            icon: "🍖",
  ...
  const stageStatusPills = useMemo(
    () => [
      ...
      },
      {
        ...
        detail:
          trustScore < 40
            ? "Low trust shows up as glances and distance."
            : trustScore < 60
              ? "Mid trust brings the dog toward center yard."
              : trustScore < 80
                ? "High trust pushes greetings to the player boundary."
                : "Very high trust unlocks intense greeting behavior.",
        tone: trustScore <= 25 ? "danger" : trustScore < 60 ? "pending" : "ok",
      },
      {
        label: "Hunger",
        value: `${hungerPct}%`,
      ...
      },
      {
        ...
        tone:
          pottySequencePhase === "cued" || pottySequencePhase === "missed"
            ? "danger"
            : pottyNeedPct >= 88
              ? "danger"
              : pottyNeedPct >= 70
          ...
          <div className="pup-card flex min-h-0 flex-1 flex-col rounded-[28px] px-3 pb-4 sm:px-6 sm:pb-6">
            <div className="flex min-h-0 flex-1 flex-col gap-4">
              <div className={statsStackClassName}>
                  ...
                  <div className="mt-3 rounded-xl border border-white/10 bg-black/15 px-3 py-3">
                    ...
                    <div className="mt-2 flex flex-wrap gap-2 text-[10px] font-bold uppercase tracking-[0.14em] text-white/70">
                      <div className="rounded-full border border-white/10 bg-black/20 px-2.5 py-1">
                        {Math.max(
                          0,
                          Number(habitLoopModel.completedCount || 0)
                        )}
                        /3 care touches
                      </div>
                      <div className="rounded-full border border-white/10 bg-black/20 px-2.5 py-1">
                ...
                <ProgressionMilestoneCard model={progressionPathModel} />
                {actionOutcomeLabel ? (
                ...
                {isApartmentEnvironment ? (
                  <div className="rounded-xl border border-amber-300/35 bg-amber-400/10 px-3 py-2 text-xs font-semibold text-amber-50">
                    Apartment hard mode: food left on the low table can be
                    stolen.
                  </div>
                ) : null}
