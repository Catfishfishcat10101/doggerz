...
import DogAIEngine from "@/components/Features/DogAIEngine.jsx";   // autonomous
import DogStage from "@/components/game/DogStage.jsx";             // view-only
...
export default function Home() {
  ...
  // Keep world sizes consistent with your stage math
  const WORLD_W = 640;
  const WORLD_H = 360;
  ...
  return (
    <div className="container mx-auto px-4 py-8 grid gap-8">
      {/* --------- Hero --------- */}
      <header aria-labelledby="home-title" data-route-title="true" className="grid gap-4">
      ...
      {/* --------- LIVE DEMO: the dog actually renders here --------- */}
      {/* Headless driver (autonomous). Mount once per page. */}
      <DogAIEngine worldW={WORLD_W} worldH={WORLD_H} debug={false} />
      <BackgroundScene skin="default" accent="#10b981">
        <DogStage />
      </BackgroundScene>

      {/* --------- Ads (optional) --------- */}
      {canShowAds(location.pathname) && <AdSlot type="banner" className="my-2" />}

      {/* --------- Value props --------- */}
      <ul className="mt-6 grid gap-4 sm:grid-cols-2">
        <Feature label="One dog per user" desc="Fair play enforced in Firestore rules. Your pup is uniquely yours." />
        <Feature label="Needs & mood system" desc="Gentle decay, no click-grind. Icons + labels (not color-only)." />
        <Feature label="Responsive & accessible" desc="High contrast, visible focus, reduced-motion friendly." />
        <Feature label="Offline-ready PWA" desc="Install to your device and play without a connection." />
      </ul>

      {/* --------- Secondary nav --------- */}
      <div className="mt-6 flex flex-wrap gap-3 text-sm text-white/70">
        <Link to={PATHS.SHOP} className="underline underline-offset-4 hover:opacity-90">Browse Shop</Link>
        <span aria-hidden="true" className="mx-1 opacity-40">•</span>
        <Link to={PATHS.PRIVACY} className="underline underline-offset-4 hover:opacity-90">Privacy</Link>
        <span aria-hidden="true" className="mx-1 opacity-40">•</span>
        <Link to={PATHS.TERMS} className="underline underline-offset-4 hover:opacity-90">Terms</Link>
      </div>
    </div>
  );
}
