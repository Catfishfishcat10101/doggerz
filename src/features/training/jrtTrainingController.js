...
function commandIdToActionId(value) {
  return String(value || "")
    .trim()
...
const SHOWOFF_COMMAND_IDS = Object.freeze([
  "spin",
  "shake",
  "rollOver",
  "sitPretty",
...
function pickShowoffAction({
  ...
  const requestedActionId = commandIdToActionId(commandId);
  const pool = (Array.isArray(unlockedIds) ? unlockedIds : [])
    .map((id) => String(id || "").trim())
    .filter(Boolean)
    .filter((id) => SHOWOFF_COMMAND_IDS.includes(id))
    .map((id) => ({
      commandId: id,
      actionId: commandIdToActionId(id),
    }))
    .filter((entry) => entry.actionId !== requestedActionId);
...
export function resolveJrtTrainingReaction({
  ...
  if (reinterpretChance > 0 && roll > 100 - reinterpretChance) {
    return {
      kind: "reinterpret",
      performedActionId: showoffAction.actionId,
      performedCommandId: showoffAction.commandId,
      requestedCommandId: commandId,
      reasonId: "showoff",
      roll,
      thresholds: {
  ...
  return {
    kind: "obey",
    performedActionId: commandIdToActionId(commandId),
