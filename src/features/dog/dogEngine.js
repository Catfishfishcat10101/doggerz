...
// Legacy/simple decay helper. The stepped simulation below is the canonical
// time engine for Doggerz runtime state; keep this exported for compatibility.
export function calculateTimeDecay(currentStats, diffHours, modifiers = {}) {
  ...
  const effectiveHours = vacationEnabled
    ? toFinite(diffHours, 0) * toFinite(vacationMultiplier, 1)
    : toFinite(diffHours, 0);

  Object.entries(nextStats).forEach(([key, rawValue]) => {
    const value = toFinite(rawValue, 0);
    const rate = DECAY_PER_HOUR[key] || 0;
    let delta =
      rate * DECAY_SPEED * effectiveHours * toFinite(stageMultiplier, 1);
...
function baseLossPerMinuteFromHoursTo40(hoursTo40) {
  const minutes = Math.max(1, toFinite(hoursTo40, 0) * 60);
  return 60 / minutes;
}
...
function applyDecay(current, baseLossPerMinute, minutes, cfg) {
  ...
  const zf = zoneFactorFn(toFinite(current, 0), zones);
  const loss =
    toFinite(baseLossPerMinute, 0) *
    zf *
    toFinite(multiplier, 0) *
    toFinite(minutes, 0);

  let next = toFinite(current, 0) - loss;

  if (!allowBelowFloor) {
    next = Math.max(toFinite(floor ?? zones.FLOOR, zones.FLOOR), next);
  } else {
...
function maybeTriggerAccident(
  ...
  const triggerAt = toFinite(tuning?.accidents?.triggerAtOrBelow, 10);
  if (toFinite(s.bladder, 0) > triggerAt) return;

  const minutesSimulatedSoFar = minutesTotal - remaining - stepMin;
  const currentSimMs =
    toFinite(startNowMs, 0) - (minutesTotal - minutesSimulatedSoFar) * 60000;

  const cooldownMs = toFinite(tuning?.accidents?.cooldownMinutes, 0) * 60000;
  if (
    toFinite(simState.lastAccidentAt, 0) &&
    currentSimMs - toFinite(simState.lastAccidentAt, 0) < cooldownMs
  ) {
    return;
  }

  simState.lastAccidentAt = currentSimMs;
  simState.messCount = toFinite(simState.messCount, 0) + 1;

  s.bladder = clamp(toFinite(tuning?.accidents?.relieveTo, 80), 0, 100);
  s.cleanliness = clamp(
    toFinite(s.cleanliness, 0) -
      toFinite(tuning?.accidents?.cleanlinessPenalty, 10),
    0,
    100
  );
  s.happiness = clamp(
    toFinite(s.happiness, 0) - toFinite(tuning?.accidents?.happinessPenalty, 6),
    0,
    100
  );
  simState.careDebt = clamp(
    toFinite(simState.careDebt, 0) +
      toFinite(tuning?.accidents?.careDebtPenalty, 4),
    0,
    toFinite(tuning?.careDebt?.cap, 100)
  );
}
...
function simulateMinutes(
  ...
  let remaining = toFinite(minutesTotal, 0);
  ...
  const baseLoss = {
    ...
    health: baseLossPerMinuteFromHoursTo40(hoursTo40.health),
  ...
  while (remaining > 0) {
    ...
    const needPressure = avg(
      ...
      urgency(s.health, zones),
      urgency(s.bladder, zones),
      urgency(s.affection, zones),
      urgency(s.mentalStimulation, zones)
    );

    const wb = clamp(toFinite(simState.wellbeing, 0), 0, 100);
    const protect =
      1.0 + (toFinite(tuning?.wellbeing?.protectMin, 0.65) - 1.0) * (wb / 100);
    const stageMult = toFinite(stage.globalDecayMultiplier, 1.0);
    const simMult = toFinite(opts?.decayMultiplier, 1.0);

    if (
      simState.sleep.mode !== "nap" &&
      toFinite(s.energy, 0) <= toFinite(tuning?.sleep?.enterNapAtOrBelow, 30)
    ) {
      simState.sleep.mode = "nap";
      simState.sleep.napMinutesLeft = toFinite(
        tuning?.sleep?.maxNapMinutes,
        90
      );
    }
    ...
    const isNapping =
      simState.sleep.mode === "nap" &&
      toFinite(simState.sleep.napMinutesLeft, 0) > 0;

    const napMult = {
      hunger: isNapping
        ? toFinite(tuning?.sleep?.hungerDecayMultiplierWhileNapping, 0.65)
        : 1.0,
      happiness: isNapping
        ? toFinite(tuning?.sleep?.happinessDecayMultiplierWhileNapping, 0.8)
        : 1.0,
      bladder: isNapping
        ? toFinite(tuning?.sleep?.bladderDecayMultiplierWhileNapping, 0.85)
        : 1.0,
      cleanliness: isNapping
        ? toFinite(tuning?.sleep?.cleanlinessDecayMultiplierWhileNapping, 1.0)
        : 1.0,
    };
    ...
    s.health = applyDecay(s.health, baseLoss.health, stepMin, {
      floor,
      zoneFactorFn: getZoneFactor,
    ...
    if (isNapping) {
      const rec =
        toFinite(stage.energyRecoverPerMinuteWhileNapping, 0.4) * stepMin;
      s.energy = clamp(toFinite(s.energy, 0) + rec, 0, 100);
      simState.sleep.napMinutesLeft = Math.max(
        0,
        toFinite(simState.sleep.napMinutesLeft, 0) - stepMin
      );

      if (
        toFinite(s.energy, 0) >= toFinite(tuning?.sleep?.wakeAtOrAbove, 70) ||
        toFinite(simState.sleep.napMinutesLeft, 0) <= 0
      ) {
    ...
    if (needPressure < 0.2) {
      simState.wellbeing = clamp(
        toFinite(simState.wellbeing, 0) +
          toFinite(tuning?.wellbeing?.regenPerMinuteWhenFine, 0.03) * stepMin,
        0,
        100
      );
    } else {
      simState.wellbeing = clamp(
        toFinite(simState.wellbeing, 0) -
          toFinite(tuning?.wellbeing?.drainPerMinute, 0.12) *
            needPressure *
            stepMin,
        0,
        100
    ...
    if (needPressure < 0.2) {
      simState.careDebt = clamp(
        toFinite(simState.careDebt, 0) -
          toFinite(tuning?.careDebt?.forgivePerMinuteWhenFine, 0.1) * stepMin,
        0,
        toFinite(tuning?.careDebt?.cap, 100)
      );
    } else {
      const extra =
        toFinite(simState.wellbeing, 0) <=
        toFinite(tuning?.wellbeing?.fragileThreshold, 10)
          ? toFinite(tuning?.careDebt?.buildExtraWhenWellbeingEmpty, 0.06)
          : 0;

      simState.careDebt = clamp(
        toFinite(simState.careDebt, 0) +
          (toFinite(tuning?.careDebt?.buildPerMinute, 0.1) * needPressure +
            extra) *
            stepMin,
        0,
        toFinite(tuning?.careDebt?.cap, 100)
...
export function computeNeedPressureFromStats(
  ...
  return avg(
    ...
    urgency(Number(s.health || 0), zones),
...
// Canonical stepped time engine used by the Redux tick wrapper.
export function simulateDogTime(
  ...
  const simState = {
    ...
    stats: {
      ...
      health: clamp(Number(dog?.stats?.health ?? 90), 0, 100),
  ...
  const dtMinutes = clamp(
    Math.floor(toFinite(dtMs, 0) / 60000),
    0,
    30 * 24 * 60
  );
  if (dtMinutes <= 0) {
  ...
  const fullSim = Math.min(
    dtMinutes,
    toFinite(tuning?.tick?.fullSimMinutes, 24 * 60)
  );
  ...
  if (extraSim > 0) {
    simulateMinutes(
      ...
      nowMs - extraSim * 60000,
      {
        decayMultiplier: toFinite(tuning?.tick?.extraSimDecayMultiplier, 0.25),
        floorOverride: toFinite(tuning?.tick?.extraSimFloor, zones.FLOOR),
