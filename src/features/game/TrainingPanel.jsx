...
export default function TrainingPanel({
  ...
  const sortedCommands = useMemo(() => {
    ...
    return list;
  }, [filteredCommands, nowMs, sortKey]);
  ...
  const filteredCount = sortedCommands.length;
  ...
  return (
    <section className="rounded-3xl border border-white/15 bg-gradient-to-b from-[#10131a] to-[#030305] p-5 shadow-[0_40px_90px_rgba(0,0,0,0.45)]">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3 text-[11px] text-white/60">
        <div className="relative w-full sm:max-w-xs">
          <input
            ...
            className="w-full rounded-full border border-white/10 bg-black/30 px-3 py-2 text-xs text-white/80 placeholder:text-white/30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/50"
          />
          {query ? (
            <button
              type="button"
              onClick={() => setQuery("")}
              className="absolute right-1 top-1/2 -translate-y-1/2 rounded-full border border-white/10 bg-white/10 px-2 py-1 text-[10px] uppercase tracking-[0.2em] text-white/70 hover:bg-white/20"
            >
              Clear
            </button>
          ) : null}
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <select
            value={sortKey}
            onChange={(e) => dispatch(setTrainingSortKey(e.target.value))}
            className="rounded-full border border-white/10 bg-black/30 px-3 py-1 text-[11px] uppercase tracking-[0.2em] text-white/70 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/50"
          >
        ...
        <span className="ml-auto text-[10px] uppercase tracking-[0.35em] text-white/40">
          {filteredCount}/{commands.length} showing
        </span>
      </div>
      ...
      <div className="mt-5 max-h-[380px] overflow-y-auto pr-1">
        ...
        {showLocked && groups.locked.length ? (
          <CommandGroup
        ...
        (!showLocked || !groups.locked.length) ? (
          <div className="rounded-2xl border border-white/10 bg-black/30 px-4 py-3 text-xs text-white/60">
            {query
              ? "No commands match your search. Try clearing filters."
              : "No commands available yet. Keep training to unlock more."}
          </div>
        ) : null}
      </div>
...
function CommandCard({
  ...
  return (
    <button
      ...
      aria-pressed={active}
      className={[
        "flex flex-col gap-2 rounded-2xl border text-left transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/50",
        active
