...
export {
  DOG_GUEST_STORAGE_KEY,
  DOG_SAVE_SCHEMA_VERSION,
  DOG_STORAGE_KEY,
  getDogStorageKey,
} from "@/store/dog/persistence.js";
...
const POTTY_GATE_INSTANT_UNLOCK_IDS = Object.freeze(["sit", "speak", "shake"]);
...
function getDogDisplayName(state) {
  return (
    String(state?.name || state?.identity?.name || "Your dog").trim() ||
    "Your dog"
  );
}
...
function recordPuppyPottySuccess(state, now = nowMs()) {
  ...
  if (potty.successCount >= potty.goal) {
    ...
    pushJournalEntry(state, {
      ...
      body: isPuppy
        ? "Your puppy now knows how to signal when nature calls. Sit, Speak, and Shake unlocked immediately."
        : "House training is finally locked in. Sit, Speak, and Shake are ready immediately.",
      timestamp: now,
    });
  }
}
  ...
  reducers: {
    ...
    giveWater(state, { payload }) {
      ...
      setCareResponse(state, {
        ...
        message:
          thirstBefore >= 55
            ? `${getDogDisplayName(state)} lapped up the fresh water.`
            : `${getDogDisplayName(state)} took a small drink before thirst became a problem.`,
        tone: "sky",
        icon: "water",
        now,
      });
    ...
    play(state, { payload }) {
      ...
      setCareResponse(state, {
        ...
        message:
          energyBefore >= 70
            ? `${getDogDisplayName(state)} bounced into play. Bond grew because the timing was right.`
            : `${getDogDisplayName(state)} enjoyed a short play burst without getting overworked.`,
        tone: "emerald",
        icon: "play",
        now,
      });
    ...
    petDog(state, { payload }) {
      ...
      setCareResponse(state, {
        ...
        message: isAdoptOnboarding
          ...
          : outcome === "PET_SIDE_EYE"
            ? `${getDogDisplayName(state)} accepted it on her terms. Trust still moved, just quietly.`
            : outcome === "PET_DOZE"
              ? `${getDogDisplayName(state)} relaxed into your touch and started to settle.`
              : outcome === "PET_ZOOMIES"
                ? `${getDogDisplayName(state)} turned attention into zoomies.`
                : `${getDogDisplayName(state)} leaned into your hand. Bond starts here.`,
    ...
    rest(state, { payload }) {
      ...
      setCareResponse(state, {
        key: "rest",
        label: "Rest",
        message: `${getDogDisplayName(state)} curled up to sleep. Recovery is part of care.`,
        tone: "sky",
        icon: "rest",
        now,
      });
    ...
    bathe(state, { payload }) {
      ...
      setCareResponse(state, {
        ...
        label: "Bath",
        message: bathNeeded
          ? `${getDogDisplayName(state)} shook it off, but feels cleaner.`
          : `${getDogDisplayName(state)} was already clean and had opinions about the extra bath.`,
    ...
    goPotty(state, { payload }) {
      ...
      setCareResponse(state, {
        ...
        label: "Potty",
        message: activeCue
          ? `${getDogDisplayName(state)} answered the cue. House training grew because the routine was clear.`
          : `${getDogDisplayName(state)} followed the potty routine. That consistency unlocks real training.`,
        tone: "emerald",
        icon: "potty",
        now,
      });
