...
const economy = createSlice({
  ...
  reducers: {
    spend: (s, { payload }) => {
      s.coins = Math.max(0, s.coins - payload);
    },
    earn: (s, { payload = 1 }) => {
      s.coins += payload;
    },
    grantGems: (s, { payload = 1 }) => {
      s.gems += payload;
    },
    dailyClaim: (s) => {
      ...
      if (today !== last) {
        s.streak =
          last && Date.now() - s.lastClaim < 24 * 60 * 60 * 1000
            ? s.streak + 1
            : 1;
        s.lastClaim = Date.now();
        s.coins += 5 + 10 * s.streak; // escalating daily reward
      }
    },
  },
});

export const { spend, earn, grantGems, dailyClaim } = economy.actions;
