...
export default function YardDogActor({
  ...
  React.useEffect(() => {
    ...
    const checkUrl = async (label, url) => {
      if (!url) return { label, url: null, ok: false, status: 0, type: null, error: 'no url' };
      try {
    ...
    const run = async () => {
      ...
      const fallbackUrl = spriteDebug?.effectiveFallbackSrc || spriteSrc || null;
  ...
  }, [debugSprite, spriteDebug?.effectiveFallbackSrc, spriteDebug?.sheetSrc, spriteSrc]);
  ...
  const speed =
    (isPlayIntent || intent === 'run' ? DEFAULTS.playSpeed : DEFAULTS.walkSpeed) * speedScale;
    ...
    (now) => {
      ...
      if (!critterNextAtRef.current) {
        critterNextAtRef.current =
          now + randRange(rng, DEFAULTS.critterSpawnMinMs, DEFAULTS.critterSpawnMaxMs);
        return;
      }
  ...
  React.useEffect(() => {
    ...
    const tick = (t) => {
        ...
        if (ageMs > 9000 || next.x < -10 || next.x > 110) {
          ...
          if (intent === "idle") {
            ...
            if (dist < 28 && rng() > 0.75) {
              ...
              targetRef.current = { x: clamp(next.x, 12, 88), y: clamp(pos.y, 90, 94) };
              lastPickRef.current = now;
            }
          }
        }
      ...
      if (intent === 'eat' && !atBowl) {
        ...
        const nextIn = randRange(rng, DEFAULTS.roamMinMs, DEFAULTS.roamMaxMs) * roamScale;
          ...
          if (intent === 'idle') {
            ...
            if (!emoteActive && now >= (idleNextEmoteAtRef.current || 0)) {
              ...
              if (rng() > 0.55) {
                ...
                idleEmoteRef.current = emotes[Math.floor(rng() * emotes.length)] || 'wag';
  ...
  const isRunning = (isPlayIntent || intent === "run") && isMoving && intensity > 0.55;
  ...
  const bubble = (() => {
    ...
    if (intent === "poop" || intent === "pee" || intent === "potty") return "...";
    return null;
  })();
      ...
      {showCritter ? (
        <div
          ...
          style={{
            ...
            opacity: 0.9,
            fontSize: 16,
            animation: reduceMotion ? "none" : "dg-hop 1.2s ease-in-out infinite",
          }}
        >
          🦋
        </div>
          ...
          {useRig ? (
            ...
            >
                ...
                {showEarOverlay ? (
                  ...
                  >
                    <svg viewBox="0 0 512 512" width="100%" height="100%" aria-hidden="true">
                      <path d="M302 132 L330 70 L354 148 Z" fill="rgba(120,72,38,0.85)" />
                      <path d="M248 138 L214 78 L198 154 Z" fill="rgba(120,72,38,0.85)" />
                      <path d="M305 144 L330 88 L345 152 Z" fill="rgba(255,255,255,0.12)" />
                      <path d="M245 150 L214 96 L206 156 Z" fill="rgba(255,255,255,0.12)" />
                    </svg>
                  </div>
                ) : null}
              {useSpritePack ? (
                  ...
                  style={{
                    ...
                    animation: reduceMotion
                      ...
                      : sleepActive
                        ...
                        : intent === 'play'
                          ? (isMoving ? 'dg-walk 0.32s ease-in-out infinite' : 'dg-sprite-bob 1.3s ease-in-out infinite')
                          : (isMoving ? 'dg-walk 0.48s ease-in-out infinite' : 'dg-sprite-bob 2.4s ease-in-out infinite'),
                  }}
                >
                  <img
                    src={spriteSrc}
              ...
              {debugSprite ? (
                ...
                >
                  <div className="font-extrabold text-emerald-200">Sprite debug</div>
                  ...
                  <div className="pointer-events-auto mt-2 flex flex-wrap items-center gap-2">
                    <button
                      ...
                      onClick={async () => {
                        ...
                        lines.push(`BASE_URL: ${String(spriteChecks?.baseUrl || import.meta.env.BASE_URL || '/')}`);
                        lines.push(`SW controller: ${spriteChecks?.sw?.controller ? 'yes' : 'no'}`);
                        lines.push(`stage: ${String(lifeStageStage || 'PUPPY')}`);
                        lines.push(`anim: ${String(spriteAnim)}`);
                        lines.push(`sheet: ${String(spriteDebug?.sheetSrc || '(none)')}`);
                        lines.push(`fallback: ${String(spriteDebug?.effectiveFallbackSrc || spriteSrc || '(none)')}`);
                        lines.push(`strip loaded: ${spriteDebug?.sheetLoaded ? 'yes' : 'no'}`);
                        lines.push(`strip failed: ${spriteDebug?.sheetFailed ? 'yes' : 'no'}`);
                        if (Array.isArray(spriteChecks?.results) && spriteChecks.results.length) {
                          lines.push('checks:');
                          for (const r of spriteChecks.results) {
                            const status = r.ok ? `OK ${r.status}` : `FAIL${r.status ? ` ${r.status}` : ''}`;
                            const extra = [r.type ? `type=${r.type}` : null, r.error ? `error=${r.error}` : null]
                              .filter(Boolean)
                              .join(' ');
                            lines.push(`- ${r.label}: ${status}${extra ? ` (${extra})` : ''}`);
                  ...
                  <div className="mt-1 text-zinc-400">
                    BASE_URL: <span className="font-semibold">{String(spriteChecks?.baseUrl || import.meta.env.BASE_URL || '/')}</span> · SW controller:{' '}
                    <span className="font-semibold">{spriteChecks?.sw?.controller ? 'yes' : 'no'}</span>
                  </div>
                  <div className="mt-1 text-zinc-200/90">
                    stage: <span className="font-semibold">{String(lifeStageStage || 'PUPPY')}</span> · anim:{' '}
                  ...
                  <div className="mt-1 break-all text-zinc-400">
                    fallback: {spriteDebug?.effectiveFallbackSrc || spriteSrc || '(none)'}
                  </div>
                  <div className="mt-1 text-zinc-400">
                    strip: {spriteDebug?.sheetLoaded ? 'loaded' : 'not loaded'} / {spriteDebug?.sheetFailed ? 'failed' : 'ok'} · frames:{' '}
                    ...
                    <div className="mt-2 space-y-1 text-zinc-300/90">
                      {spriteChecks.results.map((r) => (
                        <div key={r.label} className="break-all">
                          {r.label}:{' '}
                          <span className="font-semibold">{r.ok ? `OK ${r.status}` : `FAIL${r.status ? ` ${r.status}` : ''}`}</span>
