...
export function BottomMenuDock({
  ...
  return (
    <div className="relative overflow-hidden rounded-[30px] border border-amber-200/18 bg-[linear-gradient(180deg,rgba(8,12,24,0.88),rgba(2,6,23,0.96))] p-2 shadow-[0_20px_44px_rgba(2,6,23,0.56),0_24px_68px_rgba(2,6,23,0.34),inset_0_1px_0_rgba(255,255,255,0.14)] backdrop-blur-3xl sm:p-2.5">
      ...
      <div
        className={`${activeSection ? "mt-2.5" : ""} grid grid-cols-4 gap-1.5 sm:gap-2`}
      >
        {tabs.map((tab) => {
          ...
          return (
            ...
            >
              ...
              <span className="mt-1 block text-[10px] font-black uppercase tracking-[0.14em] sm:text-[11px]">
                {tab.label}
              </span>
            </button>
          );
        ...
        <div className="flex items-start justify-between gap-3 px-4 pt-3 pb-2 sm:px-5 sm:pt-4">
          <div>
            ...
            <div className="mt-1 text-sm text-doggerz-bone/72">
              Fast care actions sized for touch.
            </div>
          </div>
        ...
        <div className="max-h-[72dvh] overflow-y-auto px-4 pb-[calc(env(safe-area-inset-bottom,0px)+14px)] sm:px-5 sm:pb-5">
          <div className="grid grid-cols-2 gap-2.5">
            <SheetButton
              ...
              icon="🍖"
              onClick={onQuickFeed}
              tooltip="Fast hunger recovery with the existing quick-feed action."
            />
            <SheetButton
              label="Water"
              icon="💧"
            ...
            <SheetButton
              ...
              icon="🎾"
              onClick={onPlay}
              tooltip="Active play boosts happiness and bond."
            />
            <SheetButton
              label="Pet"
              icon="🖐️"
              onClick={onPet}
              tooltip="Petting steadily increases affection."
            />
            <SheetButton
              label="Bath"
              icon="🧼"
              onClick={onBath}
              tooltip="Cleaning prevents hygiene-related penalties."
            />
            <SheetButton
              label="Potty"
              icon="🌿"
              onClick={onPotty}
              tooltip={pottyTooltip || "Potty routine helps avoid accidents."}
            />
          </div>
          <div className="mt-4">
